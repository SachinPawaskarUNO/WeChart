...
class AdminController extends Controller
{
    ...
    public function searchAudios(Request $request)
    {
        if($request->ajax())
        {
            $output="";
            $rownum=0;
            $audios=audio_lookup_value::where('audio_lookup_value_tag','ilike','%'.$request->search.'%')->orderby('audio_lookup_value_tag')->paginate(6);
            if($audios)
            {
                foreach ($audios as $key => $audio) {
                    $rownum = $rownum + 1;
                    $output.='<tr>'.
                        '<td>'.$audio->audio_lookup_value_tag.'</td>'.
                        '<td>'.$audio->audio_lookup_value_link.'</td>'.
                        '<td style="text-align: right">'.
                                    '<button class="editButton btn btn-info" id='.$rownum.'><i class="fa fa-edit" aria-hidden="true"></i></button>'.
                                    '<button class="saveButton btn btn-success" id='.$rownum. ' value='.$audio->audio_lookup_value_id.'><i class="fa fa-save" aria-hidden="true"></i></button>'.
                                '</td>'.
                        '<td style="text-align: right">'.'<a href="'.route('delete_audio',[ $audio->audio_lookup_value_id]).'" class="btn btn-danger enable" id="delete" onclick="return Delete()"> <i class="fa fa-trash-o" aria-hidden="true"></i>
                                        </a>'.'</td>'.
    ...
    public function post_new_audio(Request $request)
    {
        $role='';
        if(Auth::check()) {
            $role = Auth::user()->role;
        ...
        if($role == 'Admin') {
            $audioid = $request['id'];
            $audiotag = $request['name'];
            $audiovalue = $request['link'];
            audio_lookup_value::where('audio_lookup_value_id', $audioid)->update(['audio_lookup_value_tag' => $audiotag]);
            audio_lookup_value::where('audio_lookup_value_id', $audioid)->update(['audio_lookup_value_link' => $audiovalue]);
    ...
    public function post_new_video(Request $request)
    {
        $role='';
        if(Auth::check()) {
            $role = Auth::user()->role;
        ...
        if($role == 'Admin') {
            $videoid = $request['id'];
            $videotag = $request['name'];
            $videovalue = $request['link'];
            video_lookup_value::where('video_lookup_value_id', $videoid)->update(['video_lookup_value_tag' => $videotag]);
            video_lookup_value::where('video_lookup_value_id', $videoid)->update(['video_lookup_value_link' => $videovalue]);
            //Now redirecting to orders page
            return redirect()->back();
        }
        else
    ...
    public function post_new_image(Request $request)
    {
        $role='';
        if(Auth::check()) {
            $role = Auth::user()->role;
        ...
        if($role == 'Admin') {
            $imageid = $request['id'];
            $imagetag = $request['name'];
            $imagevalue = $request['link'];

            image_lookup_value::where('image_lookup_value_id', $imageid)->update(['image_lookup_value_tag' => $imagetag]);
            image_lookup_value::where('image_lookup_value_id', $imageid)->update(['image_lookup_value_link' => $imagevalue]);
            //Now redirecting to orders page
            return redirect()->back();
        }
        else
    ...
    public function searchVideos(Request $request)
    {
        if($request->ajax())
        {
            $output="";
            $rownum=0;
            $videos=video_lookup_value::where('video_lookup_value_tag','ilike','%'.$request->search.'%')->orderby('video_lookup_value_tag')->paginate(6);
            if($videos)
            {
                foreach ($videos as $key => $video) {
                    $rownum = $rownum + 1;
                    $output.='<tr>'.
                        '<td>'.$video->video_lookup_value_tag.'</td>'.
                        '<td>'.$video->video_lookup_value_link.'</td>'.
                        '<td style="text-align: right">'.
                        '<button class="editButton btn btn-info" id='.$rownum.'><i class="fa fa-edit" aria-hidden="true"></i></button>'.
                        '<button class="saveButton btn btn-success" id='.$rownum. ' value='.$video->video_lookup_value_id.'><i class="fa fa-save" aria-hidden="true"></i></button>'.
                        '</td>'.
                        '<td style="text-align: right">'.'<a href="'.route('delete_video',[ $video->video_lookup_value_id]).'" class="btn btn-danger enable" id="delete" onclick="return Delete()"> <i class="fa fa-trash-o" aria-hidden="true"></i>
                                        </a>'.'</td>'.
    ...
    public function searchImages(Request $request)
    {
        if($request->ajax())
        {
            $output="";
            $rownum=0;
            $images=DB::table('image_lookup_value')->where('image_lookup_value_tag','ilike','%'.$request->search.'%')->orderby('image_lookup_value_tag')->paginate(6);
            if($images)
            {
                foreach ($images as $key => $image) {
                    $rownum = $rownum + 1;
                    $output.='<tr>'.
                        '<td>'.$image->image_lookup_value_tag.'</td>'.
                        '<td>'.$image->image_lookup_value_link.'</td>'.
                        '<td style="text-align: right">'.
                        '<button class="editButton btn btn-info" id='.$rownum.'><i class="fa fa-edit" aria-hidden="true"></i></button>'.
                        '<button class="saveButton btn btn-success" id='.$rownum. ' value='.$image->image_lookup_value_id.'><i class="fa fa-save" aria-hidden="true"></i></button>'.
                        '</td>'.
                        '<td style="text-align: right">'.'<a href="'.route('delete_image',[ $image->image_lookup_value_id]).'" class="btn btn-danger enable" id="delete" onclick="return Delete()"> <i class="fa fa-trash-o" aria-hidden="true"></i>
                                        </a>'.'</td>'.
